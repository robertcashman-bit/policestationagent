...
export default function Chatbot() {
  ...
  const searchWebsiteContent = useCallback(
    async (
      query: string
    ): Promise<{
      answer: string;
  ...
  const handleQuickOption = async (option: string) => {
    ...
    if (option === "police-station") {
      ...
      botResponse =
        "We provide reliable police station agent cover for law firms. Please call 01732 247427 or visit our [contact page](/contact) to discuss requirements and rates.";
    }
  ...
  return (
    <>
      ...
      {isOpen && (
        <div
          className={`fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[9999] w-[calc(100vw-2rem)] sm:w-[280px] max-w-[280px] bg-white rounded-xl shadow-2xl border border-slate-200 flex flex-col transition-all duration-300 ${isMinimized ? "h-14" : "h-[333px] sm:h-[367px]"}`}
        >
            ...
            <>
                    ...
                    <div className="flex flex-wrap gap-1">
                      {followUpQuestions.map((q, idx) => (
                        <button
                          ...
                          className="text-[9px] px-1.5 py-0.5 bg-white border border-slate-200 rounded-full hover:bg-blue-50 hover:border-blue-300 transition-all"
                        >
              ...
              <div className="bg-white border-t border-slate-200 flex-shrink-0 p-2">
                <div className="flex gap-1 items-end">
                  ...
                  <button
                    ...
                    className="bg-blue-600 hover:bg-blue-700 disabled:bg-slate-300 text-white px-2 py-1.5 rounded-lg transition-all flex-shrink-0"
                  >
                    <svg
                      xmlns="http://www.w3.org/2000/svg"
                      width="11"
              ...
              <div className="bg-slate-50 border-t border-slate-200 flex-shrink-0">
                <button
                  ...
                  className="w-full px-2 py-1 flex items-center justify-between text-[9px] text-slate-600 hover:bg-slate-100 transition-colors"
                >
