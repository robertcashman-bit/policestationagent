...
export function FirmOutreachDashboard() {
  ...
  const data = useMemo(() => {
    ...
    const unlimited =
      summary.dailyCapUnlimited === true ||
      summary.dailyCap == null ||
      summary.dailyCap >= Number.MAX_SAFE_INTEGER;
    return {
      paused: summary.paused,
      sendEnabled: summary.sendEnabled,
      dailyCap: summary.dailyCap,
      dailyCapUnlimited: unlimited,
      dailyCapLabel: unlimited ? 'unlimited' : String(summary.dailyCap),
        ...
        <div className="overflow-x-auto rounded-xl border border-[var(--border)] bg-white shadow-sm">
          <div className="border-b border-[var(--border)] px-4 py-3 space-y-3">
            ...
            <p className="text-xs text-[var(--muted)]">
              ...
              {data.dailyCapLabel} · sent today {s.sentToday ?? 0}
            </p>
