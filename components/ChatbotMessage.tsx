...
export default function ChatbotMessage({
  ...
  return (
    ...
    >
      ...
      <div className={`flex flex-col gap-1 max-w-[85%] sm:max-w-[75%] ${type === 'user' ? 'items-end' : 'items-start'}`}>
        <div
        ...
        {type === 'bot' && !isTyping && sources && sources.length > 0 && (
          <div className="flex flex-wrap gap-1 px-0.5">
            {sources.slice(0, 3).map((source, idx) => (
              ...
              >
                {source.type === 'faq' ? '📋' : '📝'} {source.title.length > 20 ? source.title.substring(0, 20) + '...' : source.title}
              </a>
            ))}
          </div>
        )}
            ...
            {onFeedback && (
              <div className="flex items-center gap-0.5 ml-0.5">
                <button
                  ...
                  aria-label="Helpful"
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="9" height="9" viewBox="0 0 24 24" fill={feedback === 'positive' ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" width="9" height="9" viewBox="0 0 24 24" fill={feedback === 'negative' ? 'currentColor' : 'none'} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
