...
function trimContent(content, targetLength) {
  ...
  if (refIndex > 0) {
    ...
    const mainText = mainContent.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
...
function ensureSingleImage(content, title) {
  ...
  if (images.length === 0) {
    // Add one image
    const imagePath = title.toLowerCase().includes('bail') ? '/blog-images/blog-listing-0.jpg' :
                     title.toLowerCase().includes('voluntary') || title.toLowerCase().includes('interview') ? '/blog-images/blog-listing-1.png' :
                     title.toLowerCase().includes('duty solicitor') ? '/blog-images/blog-listing-2.png' :
                     title.toLowerCase().includes('property') ? '/blog-images/blog-listing-3.png' :
                     title.toLowerCase().includes('caution') ? '/blog-images/blog-listing-4.png' :
