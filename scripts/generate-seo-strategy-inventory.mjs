...
function psaInventory() {
  return readJson(path.join(REPOS.psa, 'public/blog-posts.json')).map((p) => ({
    slug: p.slug,
    title: p.title,
    category: p.category || '',
...
function custodyInventory() {
  ...
  const parseTs = (file, source) => {
    const text = readText(path.join(REPOS.custodynote, file));
    for (const block of recordsBy(text, 'slug:')) {
      const slug = (block.match(/^\s*["'`]([^"'`]+)["'`]/) || [])[1];
  ...
  const imports = readJson(path.join(REPOS.custodynote, 'data/blog-imports.json'));
...
const inventory = {
  ...
  repuk: repukInventory(),
  psa: psaInventory(),
  psrtrain: psrtrainInventory(),
  custodynote: custodyInventory(),
};
