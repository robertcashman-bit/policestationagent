...
function cleanContent(content) {
  ...
  cleaned = cleaned.replace(
    /<div[^>]*class[^>]*\b(share|social|social-media|social-share)\b[^>]*>.*?<\/div>/gis,
    ""
  );
  cleaned = cleaned.replace(
    /<section[^>]*class[^>]*\b(share|social|social-media|social-share)\b[^>]*>.*?<\/section>/gis,
    ""
  );
  ...
  cleaned = cleaned.replace(
    /<[^>]+>.*?(?:facebook|twitter|linkedin|x-logo|f\s+logo|in\s+logo).*?(?:facebook|twitter|linkedin|x-logo|f\s+logo|in\s+logo).*?(?:facebook|twitter|linkedin|x-logo|f\s+logo|in\s+logo|paperclip|printer|eye).*?<\/[^>]+>/gis,
    ""
  );

  // Remove containers with social icons AND view/like counts together (the widget pattern)
  cleaned = cleaned.replace(
    /<[^>]+>.*?(?:facebook|twitter|linkedin|x-logo|paperclip|printer|eye).*?\d+\s*(?:views?|like[s]?).*?<\/[^>]+>/gis,
    ""
  );
  cleaned = cleaned.replace(
    /<[^>]+>.*?\d+\s*(?:views?|like[s]?).*?(?:facebook|twitter|linkedin|x-logo|paperclip|printer|eye).*?<\/[^>]+>/gis,
    ""
  );

  // Remove nested divs containing social widgets (multiple levels)
  cleaned = cleaned.replace(
    /<div[^>]*>.*?<[^>]+>.*?(?:facebook|twitter|linkedin|x-logo|paperclip|printer|eye).*?\d+\s*(?:views?|like[s]?|Post\s+not\s+marked).*?<\/[^>]+>.*?<\/div>/gis,
    ""
  );
  ...
  cleaned = cleaned.replace(
    /<svg[^>]*>.*?(?:facebook|twitter|linkedin|x-logo|paperclip|printer|eye|share).*?<\/svg>/gis,
    ""
  );

  // Remove anchor tags with social media hrefs or classes
  cleaned = cleaned.replace(
    /<a[^>]*(?:href|class)=[^>]*(?:facebook|twitter|linkedin|share|social)[^>]*>.*?<\/a>/gis,
    ""
  );

  // Remove spans/divs with social media icon classes
  cleaned = cleaned.replace(
    /<(?:span|div)[^>]*class[^>]*\b(share|social|emoji|facebook|twitter|linkedin)\b[^>]*>.*?<\/(?:span|div)>/gis,
    ""
  );
