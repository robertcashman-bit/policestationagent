...
async function getLocalPageContent(filePath) {
  try {
    ...
    const htmlMatch = content.match(
      /dangerouslySetInnerHTML=\{\{\s*__html:\s*([`'"])([\s\S]*?)\1\s*\}\}/
    );
    if (htmlMatch) {
...
async function getLocalNavigation() {
  ...
  for (const match of mainMenuMatches) {
    ...
    if (href && text && !href.startsWith("#") && !href.startsWith("javascript:")) {
      ...
      const isInDropdown =
        beforeMatch.includes("dropdown") || beforeMatch.includes("absolute top-full");
    ...
    if (results.incomplete.length > 0) {
      ...
      results.incomplete.forEach((item) => {
        if (typeof item === "string") {
          ...
          console.log(
            `    - ${item.route} (${item.reason || Math.round(item.similarity * 100) + "% similar"})`
          );
        }
      });
    }
