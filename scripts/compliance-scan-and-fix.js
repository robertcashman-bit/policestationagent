...
const BANNED_PATTERNS = [
  ...
  },
  {
    ...
    pattern: /24\/7.*?(?:representation|legal\s+services|immediate(?:\s+representation)?|duty\s+solicitor\s+representation|emergency\s+representation)|(?:Available|contact\s+us)\s+24\/7.*?(?:representation|legal\s+services|immediate)/gi,
  ...
  },
  {
    ...
    pattern: /(?:guaranteed\s+senior\s+solicitor|you\s+are\s+guaranteed\s+to\s+be\s+represented\s+by|guaranteed\s+to\s+be\s+represented|guarantee.*?representation|guaranteed.*?attendance)/gi,
    replacement: 'Where possible, you may be represented by Robert Cashman, subject to availability and conflicts. If Robert cannot attend, Tuckers Solicitors LLP will arrange an alternative suitably qualified representative.',
  ...
  },
  {
    ...
    replacement: 'If you are detained at a police station, you are entitled to free and independent legal advice. You can ask custody staff to contact Tuckers Solicitors LLP. You may request Robert Cashman as your named solicitor, subject to availability and conflicts.',
    description: 'Inconsistent Legal Aid eligibility messaging'
  },
  {
    id: 'ask-for-police-station-agent',
    pattern: /(?:ask\s+(?:for|the\s+police\s+for)|tell.*?you\s+want|request)\s+["']?(?:Police\s+Station\s+Agent|policestationagent)["']?\s+to\s+(?:represent|attend)/gi,
...
const SCAN_EXTENSIONS = ['.ts', '.tsx', '.js', '.jsx', '.json', '.md', '.mdx', '.txt', '.yml', '.yaml', '.html'];
