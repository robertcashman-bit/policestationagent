...
posts.forEach(post => {
  ...
  const blockPattern = /<(?:p|div|section)[^>]*>[\s\S]*?facebook[\s\S]*?❤[\s\S]*?<\/(?:p|div|section)>/i;
    ...
    for (let i = fbIndex; i >= 0; i--) {
      if (content.substring(i, i + 2) === '</') {
        ...
        if (depth === 0) {
          ...
          if (tagStart !== -1) {
            const tag = content.substring(tagStart + 1, i).split(' ')[0].toLowerCase();
    ...
    if (heartIndex !== -1) {
      ...
      for (let i = heartIndex; i < content.length; i++) {
        if (content.substring(i, i + 2) === '</') {
          ...
          if (tagEnd !== -1) {
            const tag = content.substring(i + 2, tagEnd).split(' ')[0].toLowerCase();
