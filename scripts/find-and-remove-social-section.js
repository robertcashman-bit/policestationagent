  ...
  for (const fbPattern of fbPatterns) {
    ...
    if (fbMatch) {
      ...
      if (lastOpenTag !== -1) {
        ...
        if (tagMatch && ['div', 'p', 'section', 'article', 'span', 'figure'].includes(tagMatch[1].toLowerCase())) {
      ...
      if (heartIndex !== -1 && heartIndex > fbIndex) {
        ...
        if (firstCloseTag !== -1) {
          ...
          if (closeTagEnd !== -1) {
            const tagName = afterHeart.substring(firstCloseTag + 2, closeTagEnd).split(' ')[0].toLowerCase();
        ...
        removed = true;
        console.log(`Post ID ${post.id}: Removed Facebook/heart section (${originalLength - content.length} chars)`);
        break;
      } else {
        ...
        if (closeTagMatch) {
          const endIndex = fbIndex + fbMatch[0].length + afterFb2.indexOf(closeTagMatch[0]) + closeTagMatch[0].length;
          content = content.substring(0, startIndex) + content.substring(endIndex);
          removed = true;
          console.log(`Post ID ${post.id}: Removed Facebook section (no heart found, ${originalLength - content.length} chars)`);
          break;
        }
      }
    }
