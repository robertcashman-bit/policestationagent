...
searchPatterns.forEach(pattern => {
  const posts = db.prepare(`SELECT id, title FROM blog_posts WHERE content LIKE ?`).all(`%${pattern}%`);
  if (posts.length > 0) {
...
const allPosts = db.prepare('SELECT id, title, content FROM blog_posts WHERE content LIKE ? OR content LIKE ? OR content LIKE ?').all(
  '%07897 015550%',
  '%police station representation in the Kent area%',
  '%lucide-octagon%'
);
