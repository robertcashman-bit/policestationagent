/**
 * Process HTML File From Anywhere
 * Can process a saved HTML file from any location
 */
...
function extractMetadata(html) {
  ...
  return {
    title:
      ...
      "Services for Clients | Police Station Agent",
    description: document.querySelector('meta[name="description"]')?.getAttribute("content") || "",
    canonical:
      document.querySelector('link[rel="canonical"]')?.getAttribute("href") ||
      "https://policestationagent.com/for-clients",
  };
}
...
async function processFile(filePath, route = "/for-clients") {
  try {
...
async function main() {
  ...
  if (!filePath) {
    console.log("📝 Process HTML File From Anywhere\n");
    console.log("Usage:");
    console.log('  node scripts/process-html-from-anywhere.js "path/to/file.html"');
    console.log("\nExample:");
    console.log(
      '  node scripts/process-html-from-anywhere.js "C:\\Users\\rober\\Downloads\\Services for Clients.html"'
    );
    console.log('  node scripts/process-html-from-anywhere.js "legacy\\Services for Clients.html"');
    process.exit(1);
  }
  ...
  if (success) {
    console.log("\n✅ Successfully updated /for-clients page!");
  } else {
