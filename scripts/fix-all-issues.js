...
async function scrapeMissingPages() {
  ...
  try {
    const { stdout, stderr } = await execPromise("node scripts/scrape-missing-pages.js", {
...
async function rebuildMissingPages() {
  ...
  try {
    const { stdout, stderr } = await execPromise("node scripts/auto-rebuild-all-pages.js", {
      cwd: path.join(__dirname, ".."),
      maxBuffer: 10 * 1024 * 1024,
    });
    console.log(stdout);
