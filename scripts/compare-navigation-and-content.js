  ...
  try {
    ...
    const nav = await page.evaluate(() => {
      ...
      const dropdownTriggers = document.querySelectorAll(
        '[class*="dropdown"], [class*="menu"], button[aria-expanded], a[aria-haspopup]'
      );
      dropdownTriggers.forEach((trigger) => {
        ...
        if (triggerText) {
          ...
          if (dropdown) {
            ...
            navData.dropdowns[triggerText] = items
              .map((item) => ({
  ...
  try {
    ...
    const links = await page.evaluate(() => {
      ...
      return allLinks
        ...
        .filter(
          (link) => link.href && !link.href.startsWith("#") && !link.href.startsWith("javascript:")
        );
    });
...
async function main() {
  ...
  try {
    ...
    await fs.writeFile(
      navFile,
      JSON.stringify({ navigation: psaNav, pages: psaPages }, null, 2),
      "utf-8"
    );
