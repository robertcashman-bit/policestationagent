...
const promoRegex = /<div[^>]*data-breakout[^>]*>[\s\S]*?If you require police station representation in the Kent area[\s\S]*?<\/p>[\s\S]*?<\/div>/gi;
...
if (matches) {
  ...
  const simplePattern = /<p[^>]*>[\s\S]*?If you require police station representation in the Kent area[\s\S]*?<\/p>/gi;
...
content = content.replace(/<div[^>]*data-hook="rcv-block\d+"[^>]*><\/div>\s*<div[^>]*data-hook="rcv-block\d+"[^>]*><\/div>/gi, '');
