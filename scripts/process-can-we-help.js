/**
 * Process "Can We Help You" HTML file
 */
...
function extractMetadata(html) {
  ...
  return {
    title:
      ...
      "Can We Help You? | Interactive Guide | Police Station Agent",
    description: document.querySelector('meta[name="description"]')?.getAttribute("content") || "",
    canonical:
      document.querySelector('link[rel="canonical"]')?.getAttribute("href") ||
      "https://policestationagent.com/canwehelp",
  };
}
...
async function main() {
  ...
  const targetFile = files.find((f) => f.includes("Can We Help") && f.endsWith(".html"));

  if (!targetFile) {
    console.error('❌ Could not find "Can We Help You" HTML file in Downloads');
    process.exit(1);
  }
  ...
  // Update the page
  const outputPath = path.join(APP_DIR, "canwehelp", "page.tsx");
  const existing = await fs.readFile(outputPath, "utf8");
  const escapedContent = escapeForTemplate(mainContent);
  const canonical = metadata.canonical || "https://policestationagent.com/canwehelp";
  ...
  console.log(`✅ Updated: /canwehelp`);
  console.log(`\n✅ Successfully populated /canwehelp page!`);
}
