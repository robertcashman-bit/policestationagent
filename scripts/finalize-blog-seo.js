...
function generateSEODescription(title, content) {
  ...
  const descriptions = {
    ...
    "no further action":
      "What does no further action mean after a police interview? Understand the implications and your rights. Free legal advice available.",
    "released under investigation":
      "Understanding released under investigation (RUI) versus police bail. Learn about the differences and your rights. Free legal advice available.",
  };
...
console.log(`\nSEO Verification:`);
console.log(
  `- Posts with optimized meta descriptions (100-200 chars): ${postsWithGoodMetaDesc}/${posts.length}`
);
console.log(`- Posts with images: ${postsWithImages}/${posts.length}`);
console.log(
  `- Posts with featured images: ${enhancedPosts.filter((p) => p.image).length}/${posts.length}`
);
