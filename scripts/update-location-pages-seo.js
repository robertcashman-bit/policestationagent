...
function updateLocationPage(location) {
  const filePath = path.join(__dirname, '..', 'app', `police-station-agent-${location}`, 'page.tsx');
  ...
  const locData = locations[location] || { name: location.charAt(0).toUpperCase() + location.slice(1), areas: location };
