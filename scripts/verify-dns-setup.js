  ...
  try {
    ...
    if (projects.projects) {
      for (const project of projects.projects) {
        ...
        if (domains.domains) {
          const hasDomain = domains.domains.some(d => 
            d.name === DOMAIN || d.name === WWW_DOMAIN
          );
          if (hasDomain) {
            return project.id;
          }
...
async function checkVercelDomainStatus() {
  ...
  try {
    ...
    if (rootStatus) {
      ...
      log(`   Verified: ${rootStatus.verified ? '✅ Yes' : '❌ No'}`, rootStatus.verified ? 'green' : 'red');
    ...
    if (wwwStatus) {
      ...
      log(`   Verified: ${wwwStatus.verified ? '✅ Yes' : '❌ No'}`, wwwStatus.verified ? 'green' : 'red');
    } else {
...
async function checkDNSResolution() {
  ...
  try {
    ...
    results.root = {
      resolved: true,
      addresses: addresses,
      pointsToVercel: addresses.some(addr => 
  ...
  try {
    ...
    results.www = {
      ...
      addresses: addresses,
      pointsToVercel: addresses.some(addr => 
        addr.includes('76.76.21') || 
        addr.includes('76.76.22') ||
        addr.includes('76.76.23')
      )
    };
    log(`   ✅ Resolves to: ${addresses.join(', ')}`, 'green');
...
async function checkEnvironmentVariables() {
  ...
  try {
    ...
    const siteUrlVar = envVars.envs.find(env => 
    ...
    if (siteUrlVar) {
      ...
      const isCorrect = siteUrlVar.value === `https://${DOMAIN}` || 
...
async function main() {
  ...
  if (vercelStatus.root && vercelStatus.root.status === 'valid' && 
      dnsResults.root && dnsResults.root.resolved) {
