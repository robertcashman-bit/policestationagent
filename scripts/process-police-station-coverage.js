/**
 * Process "Police Station Coverage Kent & Medway" HTML file
 */
...
function extractMetadata(html) {
  ...
  return {
    title:
      ...
      "Police Station Coverage Kent & Medway | Robert Cashman | Police Station Agent",
...
async function main() {
  ...
  const targetFile = files.find(
    (f) =>
      f.includes("Police Station Coverage Kent") &&
      f.includes("Medway") &&
      f.includes("Robert Cashman") &&
      f.endsWith(".html")
  );

  if (!targetFile) {
    console.error(
      '❌ Could not find "Police Station Coverage Kent & Medway" HTML file in Downloads'
    );
    process.exit(1);
  }
