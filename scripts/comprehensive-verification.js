...
const EXPECTED_NAVIGATION = {
  ...
  dropdowns: {
    ...
    Coverage: ["Areas Covered", "Police Stations", "Kent Police Stations", "Areas"],
    Articles: [
      "All Articles",
...
async function extractLocalNavigation() {
  ...
  while ((match = mainMenuPattern.exec(content)) !== null) {
    ...
    const isInDropdown =
      beforeMatch.includes("absolute top-full") ||
...
async function main() {
  ...
  console.log(
    `  ✅ Complete pages: ${allNavRoutes.size - missingRoutes.length - emptyRoutes.length - routes404.length}`
  );
