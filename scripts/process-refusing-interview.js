/**
 * Process "Refusing a Police Interview - Pros & Cons" HTML file
 */
...
function extractMetadata(html) {
  ...
  return {
    title:
      ...
      "Refusing a Police Interview - Pros & Cons | Expert Advice",
    description: document.querySelector('meta[name="description"]')?.getAttribute("content") || "",
    canonical:
      document.querySelector('link[rel="canonical"]')?.getAttribute("href") ||
      "https://policestationagent.com/refusingpoliceinterview",
  };
}
...
async function main() {
  ...
  const targetFile = files.find(
    (f) =>
      f.includes("Refusing a Police Interview") && f.includes("Pros & Cons") && f.endsWith(".html")
  );

  if (!targetFile) {
    console.error('❌ Could not find "Refusing a Police Interview" HTML file in Downloads');
    process.exit(1);
  }
  ...
  let route = "/refusingpoliceinterview";
