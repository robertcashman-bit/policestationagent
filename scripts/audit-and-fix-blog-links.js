...
function fixHref(href, context) {
  ...
  if (replacements[pathname]) {
    ...
    changedCounts.set(`${pathname} -> ${replacements[pathname]}`, (changedCounts.get(`${pathname} -> ${replacements[pathname]}`) || 0) + 1);
  ...
  if (pathname.startsWith('/blog/')) {
    ...
    if (rest && !rest.includes('/')) {
      ...
      if (fixed && fixed !== rest) {
        ...
        changedCounts.set(`/blog/${rest} -> /blog/${fixed}`, (changedCounts.get(`/blog/${rest} -> /blog/${fixed}`) || 0) + 1);
        return { href: next, changed: true };
      }
    }
  }
...
function main() {
  ...
  for (const post of legacy) {
    ...
    const ctx = { routeSet, dynamicMatchers, blogSlugSet, replacements, changedCounts, brokenCounts, externalCounts };
