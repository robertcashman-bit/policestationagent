...
async function checkDomain(domain) {
  ...
  try {
    ...
    if (result.status === 200) {
      ...
      // Check cache headers
      const cacheControl =
        result.headers["cache-control"] || result.headers["Cache-Control"] || "not set";
      ...
      return {
        ...
        totalLinks: EXPECTED_LINKS.length,
      };
    } else {
      console.log(`   ❌ Status: ${result.status}`);
...
async function main() {
  ...
  results.forEach((r) => {
    ...
    console.log(
      `${icon} ${r.domain.padEnd(35)} Status: ${r.status} | Links: ${linksStatus} ${r.foundLinks}/${r.totalLinks}`
    );
  });
