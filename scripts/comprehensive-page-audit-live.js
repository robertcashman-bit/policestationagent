...
async function getAllPagesFromSitemap() {
  try {
    ...
    while ((match = staticUrlPattern.exec(sitemapContent)) !== null) {
      ...
      url = url.replace(/\$\{baseUrl\}/g, '').replace(/`/g, '').trim();
      
      // Skip dynamic routes (containing ${} or variables)
      if (url.includes('${') || url.includes('post.slug') || url.includes('station.slug') || 
          url.includes('service.slug') || url.includes('slug')) {
        continue;
      }
  ...
  try {
    ...
    const mainContent = await page.evaluate(() => {
      ...
      return {
        text: cleanText.trim(),
        paragraphs,
        wordCount: cleanText.trim().split(/\s+/).filter(w => w.length > 0).length,
      };
    });
    ...
    const hasSufficientContent = contentLength >= MIN_CONTENT_LENGTH && 
                                 mainContent.paragraphs >= MIN_PARAGRAPHS;
...
async function main() {
  ...
  try {
    ...
    for (let i = 0; i < pagesArray.length; i++) {
      ...
      if (pageResult.error) {
        ...
        console.log(`⚠️  INSUFFICIENT CONTENT (${pageResult.contentLength} chars, ${pageResult.paragraphs} paragraphs)`);
        results.summary.insufficientContent++;
        results.summary.working++;
      } else {
        console.log(`✅ OK (${pageResult.contentLength} chars, ${pageResult.paragraphs} paragraphs, ${pageResult.links.length} links)`);
        results.summary.working++;
      }
      ...
      if (!pageResult.error && !pageResult.has404 && pageResult.status < 400) {
        ...
        for (const link of pageLinks.slice(0, 20)) { // Limit to 20 links per page to avoid timeout
  ...
  // List broken pages
  const brokenPages = results.pages.filter(p => p.error || p.has404 || (p.status && p.status >= 400));
  ...
  const insufficientPages = results.pages.filter(p => 
    !p.error && 
    !p.has404 && 
    p.status < 400 && 
    !p.hasSufficientContent
  );
  ...
  if (insufficientPages.length > 0) {
    ...
    insufficientPages.forEach(page => {
      console.log(`   ${page.url}`);
      console.log(`      Content: ${page.contentLength} chars, ${page.paragraphs} paragraphs, ${page.wordCount} words`);
  ...
  // List broken links
  const brokenLinks = Array.from(results.links.values()).filter(l => !l.works && l.status !== 'skipped');
  ...
  if (results.summary.broken > 0 || results.summary.insufficientContent > 0 || results.summary.linksBroken > 0) {
    process.exit(1);
  }
}
