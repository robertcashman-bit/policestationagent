...
posts.forEach((post, index) => {
  ...
  if (fbMatches[0]) {
    ...
    console.log(
      `  Context: ${content.substring(Math.max(0, fbIndex - 100), Math.min(content.length, fbIndex + 500))}`
    );
  }
  ...
  if (heartMatches[0]) {
    ...
    if (heartIndex === -1) {
      ...
      if (heartIndex2 !== -1) {
        ...
        console.log(
          `  Context: ${content.substring(Math.max(0, heartIndex2 - 500), Math.min(content.length, heartIndex2 + 200))}`
        );
      }
    } else {
      console.log(`  Heart found at index: ${heartIndex}`);
      console.log(
        `  Context: ${content.substring(Math.max(0, heartIndex - 500), Math.min(content.length, heartIndex + 200))}`
      );
    }
  }
