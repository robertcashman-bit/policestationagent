...
// Routes to test
const TEST_ROUTES = ["/", "/services", "/contact", "/blog", "/coverage"];
...
async function generateSummary(scores) {
  ...
  if (allScores.length > 0) {
    summary.summary.averagePerformance = Math.round(
      allScores.reduce((sum, s) => sum + s.mobile.performance + s.desktop.performance, 0) /
        (allScores.length * 2)
    );
    summary.summary.averageAccessibility = Math.round(
      allScores.reduce((sum, s) => sum + s.mobile.accessibility + s.desktop.accessibility, 0) /
        (allScores.length * 2)
    );
    summary.summary.averageBestPractices = Math.round(
      allScores.reduce((sum, s) => sum + s.mobile.bestPractices + s.desktop.bestPractices, 0) /
        (allScores.length * 2)
    );
...
async function main() {
  ...
  for (const route of TEST_ROUTES) {
    ...
    console.log(
      `  ✅ Mobile: P${mobileScores.performance} A${mobileScores.accessibility} BP${mobileScores.bestPractices} SEO${mobileScores.seo}`
    );
    console.log(
      `  ✅ Desktop: P${desktopScores.performance} A${desktopScores.accessibility} BP${desktopScores.bestPractices} SEO${desktopScores.seo}`
    );
  }
