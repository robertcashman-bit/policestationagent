/**
 * Process "What is a Duty Solicitor" HTML File
 */
...
function extractMetadata(html) {
  ...
  return {
    title:
      ...
      "What is a Duty Solicitor | Police Station Agent",
...
async function main() {
  ...
  try {
    ...
    const matchingFile = files.find((file) => {
      ...
      return lower.includes("duty") && lower.includes("solicitor") && file.endsWith(".html");
    });
    ...
    if (matchingFile) {
      ...
      // Determine which route this should go to
      // "What is a Duty Solicitor" likely goes to /what-is-a-criminal-solicitor
      // or could be a new page
      const lower = matchingFile.toLowerCase();
      if (lower.includes("criminal")) {
        route = "/what-is-a-criminal-solicitor";
      } else {
        route = "/what-is-a-criminal-solicitor"; // Default to this
      }
    }
  } catch (error) {
  ...
  if (!filePath) {
    console.log('📝 Process "What is a Duty Solicitor" HTML\n');
    console.log("❌ File not found in Downloads folder.");
    console.log("\n💡 Please provide the full path:");
    console.log('   node scripts/process-duty-solicitor.js "path/to/file.html"');
    process.exit(1);
  }
