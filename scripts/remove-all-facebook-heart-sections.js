...
posts.forEach((post) => {
  ...
  while (iterations < maxIterations) {
    ...
    if (heartIndex !== -1 && heartIndex > fbIndex) {
      ...
      if (closeTagMatch) {
        ...
        const fbEndMatch = afterFb.match(
          /(?:<\/a>|<\/svg>|<\/div>|<\/p>|<\/section>|<\/article>|<\/span>|<\/figure>|[\s<])/
        );
        if (fbEndMatch) {
  ...
  if (removed) {
    ...
    updatedCount++;
    console.log(
      `Post ID ${post.id}: Removed Facebook/heart section(s) (${originalLength - content.length} chars removed)`
    );
  }
});
