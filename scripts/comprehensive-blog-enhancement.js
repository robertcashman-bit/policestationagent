...
function generateComprehensiveContent(post, targetLength) {
  ...
  const existingText = existingContent.replace(/<[^>]*>/g, ' ').replace(/\s+/g, ' ').trim();
  ...
  if (currentLength < targetLength) {
    ...
    if (existingContent.includes('<h2>References</h2>')) {
      return existingContent.replace('<h2>References</h2>', additionalContent + '\n\n<h2>References</h2>');
    } else {
...
function trimContent(html, targetLength) {
  ...
  if (textContent.length > targetLength) {
    ...
    if (refIndex > 0) {
      ...
      if (mainText.length > targetLength - 500) { // Leave room for references
        // Trim main content
...
console.log(`\n=== FINAL STATISTICS ===`);
console.log(`Posts with correct length (1500-2500 chars): ${finalStats.correctLength}/${enhancedPosts.length}`);
