...
async function main() {
  ...
  console.log(
    `📋 Sitemaps: ${Object.keys(results.sitemaps).filter((d) => results.sitemaps[d].status === 200).length}/${PRODUCTION_DOMAINS.length}`
  );
  ...
  fs.writeFileSync("FORESIGHT_PARITY_AUDIT_REPORT.json", JSON.stringify(report, null, 2));
