/**
 * Process "Voluntary Police Interview Solicitor Kent" HTML file
 */
...
function extractMetadata(html) {
  ...
  return {
    title:
      ...
      "Voluntary Police Interview Solicitor Kent | FREE Legal Advice | PACE Code C",
...
async function main() {
  ...
  const targetFile = files.find(
    (f) =>
      f.includes("Voluntary Police Interview Solicitor Kent") &&
      f.includes("PACE Code C") &&
      f.endsWith(".html")
  );

  if (!targetFile) {
    console.error(
      '❌ Could not find "Voluntary Police Interview Solicitor Kent" HTML file in Downloads'
    );
    process.exit(1);
  }
  ...
  let route = "/voluntary-interviews";
