...
async function extractPageData(page, url) {
  try {
    ...
    const unwanted = mainContent.querySelectorAll(
      "script, style, nav, footer, header, .header, .footer, .nav"
    );
    ...
    const links = Array.from(document.querySelectorAll("a[href]"))
      .map((a) => {
        ...
        if (
          normalized.startsWith(PSA_BASE_URL) ||
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            const titleMatch =
              content.match(/title:\s*(["'`])((?:\\.|(?!\1)[^\\])*)\1/) ||
              content.match(/title:\s*([^,\n}]+)/);
            const descMatch =
              content.match(/description:\s*(["'`])((?:\\.|(?!\2)[^\\])*)\2/) ||
              content.match(/description:\s*([^,\n}]+)/);
            const canonicalMatch =
              content.match(/canonical:\s*(["'`])((?:\\.|(?!\3)[^\\])*)\3/) ||
              content.match(/canonical:\s*([^,\n}]+)/);

            const title = titleMatch
              ? (titleMatch[2] || titleMatch[1] || "").replace(/['"]/g, "")
              : "";
            const description = descMatch
              ? (descMatch[2] || descMatch[1] || "").replace(/['"]/g, "")
              : "";
            const canonical = canonicalMatch
              ? (canonicalMatch[2] || canonicalMatch[1] || "").replace(/['"]/g, "")
              : "";
...
function extractMainContent(html) {
  ...
  const unwanted = main.querySelectorAll(
    "script, style, nav, footer, header, .header, .footer, .nav, .menu"
  );
  unwanted.forEach((el) => el.remove());
...
async function createPageFile(route, pageData, isUpdate = false) {
  ...
  try {
    ...
    const description = pageData.metaDescription.replace(
...
async function generateFinalReport() {
  ...
  const report = {
    ...
    verification: {
      status:
        parityMatrix.presentOnPSAOnly.length === 0 && parityMatrix.incompletePages.length === 0
          ? "COMPLETE"
          : "INCOMPLETE",
      message:
        parityMatrix.presentOnPSAOnly.length === 0 && parityMatrix.incompletePages.length === 0
