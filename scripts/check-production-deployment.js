...
async function main() {
  ...
  if (result.data.deployments && result.data.deployments.length > 0) {
    ...
    console.log(`   Commit: ${prodDeployment.meta?.gitlabCommitSha || prodDeployment.meta?.githubCommitSha || prodDeployment.meta?.gitCommitSha || 'N/A'}`);
    console.log(`   Branch: ${prodDeployment.meta?.gitlabCommitRef || prodDeployment.meta?.githubCommitRef || prodDeployment.meta?.gitBranch || 'N/A'}`);
    console.log(`   Message: ${prodDeployment.meta?.gitlabCommitMessage || prodDeployment.meta?.githubCommitMessage || prodDeployment.meta?.gitCommitMessage || 'N/A'}`);
    ...
    if (allResult.data.deployments) {
      allResult.data.deployments.forEach((d, i) => {
        const commit = d.meta?.gitlabCommitSha || d.meta?.githubCommitSha || d.meta?.gitCommitSha || 'N/A';
        const target = d.target || 'preview';
        const icon = target === 'production' ? '🎯' : '🔍';
        console.log(`   ${icon} ${d.url} - ${target} - Commit: ${commit.substring(0, 7)} - ${new Date(d.created).toLocaleString()}`);
      });
    }
