...
test.describe('Blog Generator E2E Pipeline', () => {
  ...
  test('4️⃣ AI Image Verification', async ({ page }) => {
    ...
    try {
      const imagePreview = page.locator('img[alt*="' + TEST_BLOG_CONFIG.topic.substring(0, 20) + '"], img.object-cover');
  ...
  test('5️⃣ Publish Blog Post', async ({ page, request }) => {
    ...
    const responsePromise = page.waitForResponse(resp => 
      resp.url().includes('/api/admin/posts') && resp.request().method() === 'POST'
    );
