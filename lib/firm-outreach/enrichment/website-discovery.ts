/**
 * Serper (Google) homepage discovery for firm prospects.
 *
 * Mirrors Policestationrepuk/lib/firm-outreach/enrichment/website-discovery.ts.
 * policestationagent has no custody-discovery module, so the minimal Serper
 * client is inlined here instead of imported from '@/lib/custody-discovery/search'.
 */
...
export async function serperSearch(query: string): Promise<SerperWebsiteHit[]> {
