...
export function getPublishedBlogPosts(): BlogPostSummary[] {
  try {
    ...
    console.log(
      `[blog.ts] Merged ${allPosts.length} total posts (${dbPosts.length} from DB, ${jsonPosts.length} from JSON)`
    );
    ...
    const normalizedPosts: BlogPostSummary[] = allPosts.map((post) => {
      ...
      const title = post.title && post.title.trim() ? post.title.trim() : "Untitled Post";
  ...
  try {
    ...
    try {
      ...
      const post = posts.find((p) => {
        ...
        return (
          postNormalizedSlug === normalizedInputSlug ||
    ...
    console.log(
      `[blog.ts] getPostBySlug: No post found for slug "${slug}" (normalized: "${normalizedInputSlug}")`
    );
    return null;
  } catch (error) {
  ...
  try {
    ...
    for (const post of posts) {
      ...
      try {
        ...
        result.updated++;
        console.log(
          `[blog.ts] Backfilled slug for ID ${post.id}: "${post.slug}" -> "${finalSlug}"`
        );
      } catch (err) {
    ...
    db.close();
    console.log(
      `[blog.ts] Backfill complete: ${result.updated} updated, ${result.skipped} skipped`
    );
  } catch (error) {
