...
export function formatMessage(content: string): string {
  ...
  formatted = formatted.replace(/\[([^\]]+)\]\(([^)]+)\)/g, '<a href="$2" class="text-blue-600 hover:text-blue-700 underline" target="_blank" rel="noopener noreferrer">$1</a>');
...
export function isUrgentQuery(query: string): boolean {
  const urgentKeywords = [
    'arrested', 'arrest', 'custody', 'in custody', 'police station',
    'urgent', 'emergency', 'immediately', 'now', 'help', 'need solicitor'
  ];
 ...
 */
export function generateFollowUpQuestions(lastMessage: string, conversationHistory: Array<{ type: string; content: string }>): string[] {
