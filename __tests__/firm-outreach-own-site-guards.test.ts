...
describe('isOwnSiteUrl', () => {
  ...
  it('exposes the canonical own-site domain list', () => {
    expect(OWN_SITE_DOMAINS).toContain('policestationagent.com');
...
describe('cleanupNonFirmProspectEmails own-site reset', () => {
  ...
  it('resets an own-site prospect, clearing email/website and moving to discovered', async () => {
    const ownSiteProspect = {
      ...
      createdAt: '',
      updatedAt: '',
      email: 'directory@policestationagent.com',
      emailConfidence: 'medium' as const,
      emailScore: 40,
      alternativeEmails: [{ email: 'alt@policestationagent.com', score: 30, confidence: 'low' as const }],
      websiteUrl: 'https://policestationagent.com/legal-services-directory/category/solicitors',
    };
