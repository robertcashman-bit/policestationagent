...
describe("editorial audit rules", () => {
  ...
  it('flags we-are-the-police claims but not brand "Who we are Police Station Agent"', () => {
    ...
    expect(
      scanText("Who we are Police Station Agent is Robert Cashman's practice for Kent cover.").some(
        (f) => f.code === "we-are-the-police"
      )
    ).toBe(false);
  });
...
describe("editorial audit cron schedule", () => {
  ...
  it("allows digest at 06:00 UTC in winter GMT (not only BST)", async () => {
    const { shouldSendDailyAudit: realShouldSend } = await vi.importActual<
      typeof import("@/lib/editorial-audit/daily-notify")
    >("@/lib/editorial-audit/daily-notify");
    // Mid-winter weekday at cron fire time: 06:00 UTC === 06:00 Europe/London
    expect(realShouldSend(new Date("2026-01-14T06:00:00.000Z"))).toBe(true);
    // Summer BST: 06:00 UTC === 07:00 London
    expect(realShouldSend(new Date("2026-07-15T06:00:00.000Z"))).toBe(true);
    // Before send window
    expect(realShouldSend(new Date("2026-01-14T05:59:00.000Z"))).toBe(false);
  });
});
