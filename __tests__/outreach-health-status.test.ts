...
describe('classifyWorkspaceHealth', () => {
  ...
  it('FAILED when eligible exist but scheduler stale and nothing accepted', () => {
    expect(
      classifyWorkspaceHealth({
        capacity: cap(),
  ...
  it('does not FAILED when scheduler is stale but sends were accepted', () => {
    expect(
      classifyWorkspaceHealth({
        capacity: cap(),
        lastSchedulerOk: true,
        lastSchedulerAgeMs: 4 * 60 * 60_000,
        outstandingFaults: [],
        providerAuthOk: true,
        acceptedToday: 300,
        temporaryFailures: 0,
